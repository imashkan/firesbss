...
local function create_group(msg)
        ...
        if is_sudo(msg) or is_realm(msg) and is_admin(msg) then
                ...
                return 'گروه [ '..string.gsub(group_name, '_', ' ')..' ] ساخته شد مبارکه.'
        end
end
...
local function create_realm(msg)
        ...
        if is_sudo(msg) or is_realm(msg) and is_admin(msg) then
                local group_creator = msg.from.print_name
                create_group_chat (group_creator, group_name, ok_cb, false)
                return 'گروه مخصوص [ '..string.gsub(group_name, '_', ' ')..' ] ساخته شد مبارکه.'
        end
end
...
local function username_id(cb_extra, success, result)
   ...
   for k,v in pairs(result.members) do
      ...
      if vusername == member then
        ...
        member_id = v.id
        if mod_cmd == 'اضافه کردن ادمین' then
            return admin_user_promote(receiver, member_username, member_id)
        elseif mod_cmd == 'حذف کردن ادمین' then
...
function run(msg, matches)
	...
	if matches[1] == 'افراد and is_momod(msg) then
    ...
    if matches[1] == 'ساخت گروه مخصوص' and matches[2] then
        group_name = matches[2]
        group_type = 'realm'
	...
	if matches[2] then if data[tostring(matches[2])] then
		...
		if matches[1] == 'گذاشتن اطلاعات' and matches[2] then
			local target = matches[2]
		    local about = matches[3]
		    return set_description(msg, data, target, about)
		end
		if matches[1] == 'گذاشتن قوانین' then
		    rules = matches[3]
			local target = matches[2]
		...
		if matches[1] == 'تنظیمات' and data[tostring(matches[2])]['settings'] then
		    ...
		    return show_group_settings(msg, data, target)
                ...
                if matches[1] == 'گذاشتن اسم' and is_realm(msg) then
		...
		if matches[1] == 'گذاشتن اسم گروه' and is_admin(msg) then
    	...
    	if matches[1] == 'راهنما' and is_realm(msg) then
     		...
     		return help()
              ...
              if matches[1] == 'گذشتن' then
                ...
                if matches[1] == 'پاک کردن' and matches[2] == 'realm' then
                  if not is_admin(msg) then
                     return nil
                  end
                  if is_realm(msg) then
		...
		if matches[1] == 'حذف کردن ادمین' then
			if string.match(matches[2], '^%d+$') then
				local admin_id = matches[2]
		...
		if matches[1] == 'وضعیت گپ'then
                        local group_type = get_group_type(msg)
			return group_type
		end
		if matches[1] == 'لیست' and matches[2] == 'admins' then
			return admin_list(msg)
		end
		if matches[1] == 'لیست' and matches[2] == 'groups' then
		...
		if matches[1] == 'لیست' and matches[2] == 'realms' then
                  if msg.to.type == 'chat' then
			realms_list(msg)
...
return {
  patterns = {
    ...
    "^(تنظیمات) (%d+)$",
        ...
        "^(وضعیت)$",
    "^(پاک کردن) (چت) (%d+)$",
    "^(پاک کردن) (گروه مخصوص) (%d+)$",
    "^(اضافه کردن ادمین) (.*)$", -- sudoers only
    "^(حذف کردن ادمین) (.*)$", -- sudoers only
    "^(لیست) (.*)$",
        "^(وضعیت گپ)$",
        "^(راهنما)$",
        "^!!tgservice (.+)$",
